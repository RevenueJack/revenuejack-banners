var container, exitBtn, legalBtn, tlMain, width=300, height=600;
...
init = function() { //called from index.html, on pageload
	...
	tlMain = new TimelineLite()
		...
		.set('.chairGroup',		{scale:1.8, x:120, y:365})
		.set('.teaTrail',		{left:20, top:130})
		.set('.teaDrip',		{left:20, top:130})
		.set('.teaDrip2',		{left:27, top:170, scaleX:0.7, scaleY:0.8})
		.set('.txt3a',			{transformOrigin:'104px 189px'})
		.set('.txt3b',			{transformOrigin:'191px 189px'})
		.set('.txt3c',			{transformOrigin:'150px 241px'})
		.set('.cta', 			{left:33, top:444, width:237, height:56, backgroundColor:"#ee7600", borderRadius:"4px", overflow:'hidden'})
	...
	function step2() {
		return new TimelineLite()
			.to('.chairGroup',  		0.9, {scale:0.8, x:-width/3, y:75, ease:Power3.easeInOut}, 0)
			.to('.f1',  				0.9, {scale:0.9, x:-width/2, y:-15, ease:Power3.easeInOut}, 0)
			.to('.f1',  				0.3, {autoAlpha:0, ease:Power1.easeInOut}, 0.25)
			.fromTo('.tea', 			0.9, {x:width, rotation:0.01, scale:1.8, y:-90}, {scale:0.95, x:0, y:11, rotation:0, ease:Power2.easeInOut}, 0)
			.to('.tea',					4.7, {x:-1, y:11, scale:0.95, rotation:-0.01, ease:Power1.easeOut}, 0.74)
